...
"""
    ...
    WorkQueue.Task<?> upX1 = task(INTERACTIVE.getName(), uploadPackTask(PROJECT_X, USER_A));
    WorkQueue.Task<?> upX2 = task(INTERACTIVE.getName(), uploadPackTask(PROJECT_X, USER_A));
    WorkQueue.Task<?> upX3 = task(INTERACTIVE.getName(), uploadPackTask(PROJECT_X, USER_A));

    WorkQueue.Task<?> rpX1 = task(INTERACTIVE.getName(), receivePackTask(PROJECT_X, USER_A));
    WorkQueue.Task<?> rpX2 = task(INTERACTIVE.getName(), receivePackTask(PROJECT_X, USER_A));
    WorkQueue.Task<?> rpX3 = task(INTERACTIVE.getName(), receivePackTask(PROJECT_X, USER_A));

    assertTrue(taskQuotas.isReadyToStart(upX1));
    startAndCompleteTask(taskQuotas, upX1);

    assertTrue(taskQuotas.isReadyToStart(rpX1));
    startAndCompleteTask(taskQuotas, rpX1);

    assertTrue(taskQuotas.isReadyToStart(upX2));

    assertTrue(taskQuotas.isReadyToStart(rpX2));
    startAndCompleteTask(taskQuotas, rpX2);

    assertFalse(taskQuotas.isReadyToStart(upX3));
    taskQuotas.onNotReadyToStart(upX3);

    assertTrue(taskQuotas.isReadyToStart(rpX3));
    startAndCompleteTask(taskQuotas, rpX3);

    startAndCompleteTask(taskQuotas, upX2);

    assertTrue(taskQuotas.isReadyToStart(upX3));
    startAndCompleteTask(taskQuotas, upX3);
  }
