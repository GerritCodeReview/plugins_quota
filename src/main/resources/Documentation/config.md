...
: Optional custom message returned when a size quota in this namespace is exceeded,
  in a context where the requested size was not known not upfront.
  This is the case when the quota is used as a pr-flight check for cases when the requested
  number of bytes  is not known in advance. For example, when a commit is received by Gerrit its
  size is not known until the transfer.

<a id="sizeLimitExceededKnownRequestedMsg" />
`quota.<namespace>.sizeLimitExceededKnownRequestedMsg`
: Optional custom message returned when a size quota in this namespace is exceeded,
  in a context where the requested number of bytes was known upfront. This is the case
  when the quota is checked as a post-receive hook or at repository creation time.
...
```
  [quota "android"]
    minStartForQueue = 5 SSH-Interactive-Worker
```

The configuration reserves 5 threads from the interactive queue exclusively for tasks
related to the android project. One must ensure the total number of reserved threads does
not exceed the queue's capacity. If this limit is surpassed, some of the configured
