...
public class ConcurrentRestApiLimiter extends AllRequestFilter {
  ...
  public ConcurrentRestApiLimiter(
      AccountLimitsFinder accountLimitsFinder,
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (isRest(request) && user.get().isIdentifiedUser()) {
      accountLimitsFinder
          .firstMatching(AccountLimitsConfig.Type.CONCURRENT_RESTAPI, user.get().asIdentifiedUser())
          .ifPresentOrElse(r -> r.getValue(), );
