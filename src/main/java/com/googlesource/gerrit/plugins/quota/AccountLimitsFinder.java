      ...
      for (String groupName : limits.get().keySet()) {
        try {
          ...
          if (!maybeInternalGroup.isPresent()) {
            logger.atSevere().log("Ignoring limits for non-internal group ''%s'' in quota.config", groupName);
          } else if (memberShip.contains(maybeInternalGroup.get().getGroupUUID())) {
            return Optional.ofNullable(limits.get().get(groupName));
          }
        } catch (ResourceNotFoundException e) {
          logger.atSevere().log("Ignoring limits for unknown group ''%s'' in quota.config", groupName);
        } catch (AuthException e) {
          logger.atSevere().log("Ignoring limits for non-visible group ''%s'' in quota.config", groupName);
        }
      }
    }
