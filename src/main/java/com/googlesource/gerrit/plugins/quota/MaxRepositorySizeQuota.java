...
public class MaxRepositorySizeQuota
  ...
  protected Optional<Long> getMaxPackSize(Project.NameKey project) {
    ...
    try {
      ...
      logger.atWarning().withCause(e).log("Couldn't calculate maxPackSize for " + project.get());
      return Optional.empty();
    }
  }
  ...
  public void onPostReceive(ReceivePack rp, Collection<ReceiveCommand> commands) {
    ...
    if (needPack(commands)) {
      try {
        ...
        logger.atWarning().withCause(e).log("Couldn't process onPostReceive for " + project.get());
      }
    }
  }
  ...
  public void set(Project.NameKey p, long size) {
    try {
      ...
      logger.atWarning().withCause(e).log("Error setting the size of project " + p.get());
    }
  }
}
