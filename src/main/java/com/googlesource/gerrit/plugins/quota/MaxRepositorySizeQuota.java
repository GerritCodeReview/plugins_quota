...
public class MaxRepositorySizeQuota
  ...
  protected Optional<Long> getMaxPackSize(Project.NameKey project) {
    ...
    try {
      ...
      log.atWarning().withCause(e).log("Couldn't calculate maxPackSize for %s", project);
      return Optional.empty();
    }
  }
  ...
  public void onPostReceive(ReceivePack rp, Collection<ReceiveCommand> commands) {
    ...
    if (needPack(commands)) {
      try {
        ...
        log.atWarning().withCause(e).log("Couldn't process onPostReceive for %s", project.get());
      }
    }
  }
  ...
  public long get(Project.NameKey p) {
    try {
      ...
      log.atWarning().withCause(e).log("Error creating RepoSizeEvent");
      return 0;
    }
  }
  ...
  public void set(Project.NameKey p, long size) {
    try {
      ...
      log.atWarning().withCause(e).log("Error setting the size of project %s", p);
    }
  }
}
