...
public class TaskQuotas implements WorkQueue.TaskParker {
  ...
  private final Pattern PROJECT_PATTERN = Pattern.compile("/.*\\.git");
  ...
  private void initQuotas() {
    quotasByNamespace.putAll(
        quotaFinder.getQuotaNamespaces().stream()
            ...
            .map(
                project -> {
                  return quotasByNamespace.getOrDefault(
                      Optional.of(quotaFinder.firstMatching(project))
                          .orElse(quotaFinder.getGlobalNamespacedQuota()),
                      List.of());
                })
            .orElse(List.of());
  ...
  private Optional<Project.NameKey> estimateProject(WorkQueue.Task<?> task) {
    ...
    return matcher.find() ? Optional.of(Project.nameKey(matcher.group())) : Optional.empty();
  }
}
