...
public class MaxConnectionsLimiter extends AllRequestFilter {
  ...
  private final Map<String, Limit> limitsByGroup = new HashMap<>();
  ...
  void init(ProjectCache projectCache) {
    ...
    for (String group : cfg.getSubsections(AccountLimitsConfig.GROUP_SECTION)) {
      ...
      if (matcher.find()) {
        groups.add(group);
        limitsByGroup.put(group, new Limit(group, Integer.parseInt(matcher.group())));
      }
    }
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (isRestApiRequest(request)) {
      ...
      if (currentUser.isIdentifiedUser()) {
        ...
        Optional<Integer> limit = getEffectiveRestApiLimit(currentUser);
  ...
  private Optional<Integer> getEffectiveRestApiLimit(CurrentUser user) {
    List<Integer> result = new ArrayList<>();

    accountLimitsFinder
        .firstMatching(groups, user.asIdentifiedUser())
        .ifPresent(g -> result.add(limitsByGroup.get(g).restApiLimit()));
