...
public class MinStartForTaskForQueueQuota {
  ...
  public Optional<TaskQuota> build(QuotaSection qs, String cfg) {
    ...
    if (matcher.matches()) {
      ...
      //System.out.println("Config parameters: reservation %s, criteria %s, queue %s, namespace %s", reservation, taskMatchCriteria, queue, qs.getNamespace());
      ...
      final Pattern taskPattern =
          (taskMatchCriteria.startsWith("^") && taskMatchCriteria.endsWith("$"))
              ? Pattern.compile(taskMatchCriteria)
              : null;
