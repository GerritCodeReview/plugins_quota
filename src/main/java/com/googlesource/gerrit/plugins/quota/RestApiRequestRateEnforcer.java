...
@Singleton
public class RestApiRequestRateEnforcer extends AllRequestFilter {
  private static final Logger log = LoggerFactory.getLogger(RestApiRequestRateEnforcer.class);
  ...
  public void doFilter(ServletRequest req, ServletResponse res, final FilterChain chain)
    ...
    if (isRest(req)) {
      Holder rateLimiterHolder = new Holder(null);
      CurrentUser u = user.get();
      Account.Id accountId = null;
      if (u.isIdentifiedUser()) {
        accountId = u.asIdentifiedUser().getAccountId();
        try {
      ...
      if (!rateLimiterHolder.inGracePeriod()
  ...
  protected boolean isRest(ServletRequest req) {
    return resturi.matcher(getURI(req)).matches();
  }
}
