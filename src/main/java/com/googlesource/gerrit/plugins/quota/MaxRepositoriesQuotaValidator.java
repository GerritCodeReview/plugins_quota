...
public class MaxRepositoriesQuotaValidator implements
  ...
  public void validateNewProject(CreateProjectArgs args) throws ValidationException {
    ...
    if (quotaSection != null) {
      if (quotaSection.getNames().contains(QuotaSection.KEY_MAX_PROJECTS)) {
        int maxRepos = quotaSection.getInt(QuotaSection.KEY_MAX_PROJECTS, Integer.MAX_VALUE);
        int count = 0;
        for (Project.NameKey p : projectCache.all()) {
          if (quotaSection.getNamespace().matches(p)) {
            count++;
          }
        }
        if (count >= maxRepos) {
          ...
          msg.append(quotaSection.getNamespace().get());
