...
public class Publisher implements Runnable {
  ...
  public Publisher(DynamicSet<UsageDataPublishedListener> listeners, ProjectCache projectCache,
      ...
      @Named(MaxRepositorySizeQuota.PUSH_COUNT_CACHE) Cache<Project.NameKey, AtomicLong> pushCountCache,
      @Named(MaxRepositorySizeQuota.FETCH_COUNT_CACHE) Cache<Project.NameKey, AtomicLong> fetchCountCache) {
  ...
  public void run() {
    ...
    try {
      ...
      RepoEvent pushCountEvent = createPushCountEvent();
      RepoEvent fetchCountEvent = createFetchCountEvent();
  ...
  private RepoEvent createPushCountEvent() throws ExecutionException {
    RepoEvent event = new RepoEvent(PUSH_COUNT);
    for (Project.NameKey p : projectCache.all()) {
        AtomicLong ifPresent = numberOfPushesCache.getIfPresent(p);
        if (ifPresent != null) {
          event.addData(ifPresent.get(), p.get());
          numberOfPushesCache.put(p, new AtomicLong(0));
        }
    }
    return event;
  }

  private RepoEvent createFetchCountEvent() throws ExecutionException {
    RepoEvent event = new RepoEvent(FETCH_COUNT);
    for (Project.NameKey p : projectCache.all()) {
        AtomicLong ifPresent = numberOfFetchesCache.getIfPresent(p);
        if (ifPresent != null) {
          event.addData(ifPresent.get(), p.get());
          numberOfFetchesCache.put(p, new AtomicLong(0));
        }
    }
    return event;
  }
