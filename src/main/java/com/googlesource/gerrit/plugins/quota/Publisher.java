  ...
  public void run() {
    ...
    for (UsageDataEventCreator creator : creators) {
      try {
        ...
        log.atWarning().withCause(e).log("Exception in usage data event creator %s", creatorName);
      }
    }
    ...
    for (UsageDataPublishedListener l : listeners) {
      try {
        ...
        log.atWarning().withCause(e).log("Exception in UsageDataPublishedListener");
      }
    }
  }
}
